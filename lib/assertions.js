...
module.exports = {
  ...
  registerSupporter: async () => {
    const supporter = await admin.auth.getUserByEmail('supporter@emulator.com')
    const data = await admin.fs.collection('supporters').doc(supporter.uid).get()
    if (!data.exists) throw new Error('\n Fail! \n Supporter entry missing from firestore')
    if (data.data().uid !== supporter.uid) throw new Error(`\n Fail! \n Error in firestore data: uid is ${data.data().uid}`)
    if (data.data().email !== supporter.email) throw new Error(`\n Fail! \n Error in firestore data: uid is ${data.data().email}`)
    if (data.data().discord === "") throw new Error(`\n Fail! \n Error in firestore data: uid is ${data.data().email}`)
    if (data.data().username !== supporter.displayName) throw new Error(`\n Fail! \n Error in firestore data: uid is ${data.data().username}`)
    else console.log('\n Pass! \n UID, Email, DiscordLink, Username all set on supporter registration')
  },
