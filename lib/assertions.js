...
module.exports = {
  ...
  onSlotBooked: async (slot) => {
    ...
    let task = await admin.fs
      ...
      .doc(slot.id)
      .get()
    if (!task.exists) throw new Error('\n Fail! Missing slot-start task.')
    else console.log('\n Pass! \n Booked value incremented, chat room created, and slot-start task scheduled.')
  },

  onSlotActive: async (buyerUid, slotId) => {
    ...
    let task = await admin.fs
      ...
      .get()
    if (!task.exists) throw new Error('\n Fail! Task missing.')
    else console.log('\n Pass! Notification fired to buyer and task scheduled for capture!')
  },
  ...
  publishedToCancelled: async (sessionId) => {
    let slots = await admin.fs
      .collection('sessions')
      .doc(sessionId)
      .collection('slots')
      .get()
    if (slots.size > 0) throw new Error('\n Fail! Slots should be deleted.')
    let chat = await admin.fs
      .collection('chats')
      .doc(sessionId)
      .get()
    if (chat.exists) throw new Error('\n Fail! Chatroom should be deleted.')
    let ses = await admin.fs
      .collection('sessions')
      .doc(sessionId)
      .get()
    if (ses.exists) throw new Error('\n Fail! Session should be deleted.')
    else console.log('\n Pass! \n Seller cancelling unbooked session successful.')
  },

  publishedToCancelled_booked: async (sessionId) => {
    let cq = await admin.fs
      .collection('cancellations')
      .where('parentSession', '==', sessionId)
      .get()
    if (cq.size !== 1) throw new Error('\n Fail! Cancellations missing.')
    let slots = await admin.fs
      .collection('sessions')
      .doc(sessionId)
      .collection('slots')
    ...
    let ses = await admin.fs
      ...
      .get()
    if (ses.exists) throw new Error('\n Fail! Session should be deleted.')
    let buyerNotifs = await admin.fs
      .collection('notifs')
      .get()
    if (buyerNotifs.empty) throw new Error('\n Fail! Notification missing.')
    let notif = await admin.fs
      .collection('notifs')
      .doc(buyerNotifs.docs[0].id)
      .collection('notif')
      .get()
    if (notif.empty) throw new Error('\n Fail! Notification missing.')
    if (notif.docs[0].data().category !== 'Cancellation') throw new Error('\n Fail! error in notification data.')
  ...
  fullToCancelled: async (sessionId) => {
    let cq = await admin.fs
      .collection('cancellations')
      .get()
    if (cq.size !== 3) throw new Error('\n Fail! Cancellations missing.')
    let notif = await admin.fs
      .collection('notifications')
      .get()
    if (notif.size !== 4) throw new Error('\n Fail! Notification missing.')
    let slots = await admin.fs
      .collection('sessions')
      .doc(sessionId)
      .collection('slots')
