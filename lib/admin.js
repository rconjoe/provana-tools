...
admin.initializeApp({ projectId: 'pv-dev-4e2c2' })
...
module.exports = {
  ...
  mockSupporter: async (n) => {
    ...
    for (i=1; i<=n.length; i++) {
      let supporter
      let testSupporter = await clientauth.auth.fetchSignInMethodsForEmail(`supporter${i}@emulator.com`)
      if (testSupporter.length > 0) i = i+n
  ...
  bookSlot: async (slotId) => {
    let supporter = await module.exports.mockSupporter([1])
    let slotToBook = await fs.collectionGroup('slots')
      .where('id', '==', slotId)
      .get()
    await slotToBook.docs[0].ref.update({
      status: 'booked',
      buyerUid: supporter[0].uid,
      buyerUsername: supporter[0].username,
      paymentIntent: 'pi_123'
    })
  }
